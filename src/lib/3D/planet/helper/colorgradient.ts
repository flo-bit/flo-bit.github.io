...
type ColorGradientBetweenOptions = (
  | number
  | Color
  | ColorGradient
...
type ColorGradientStopOptions =
  ...
  | [
      number,
      number | Color | ColorGradient | ColorGradientOptions,
      ModifierFunction
    ];
...
export class ColorGradient {
  ...
  addBetween(
    arr: ColorGradientBetweenOptions,
    min: number = -1,
    max: number = 1
  ) {
    if (arr.length < 2)
  ...
  addStop(stop: ColorGradientStopOptions) {
    ...
    if (typeof mixObject == "number") {
      ...
      !(mixObject instanceof Color) &&
      !(mixObject instanceof ColorGradient)
    ) {
  ...
  mix(
    ...
    if (i < 0 || i >= this.stops.length || j < 0 || j >= this.stops.length)
      return;
  ...
  get(
    ...
    return (
      this.mix(this.stops.length - 1, this.stops.length - 1, 0, y, z, w) ??
      noColor
    );
  }
